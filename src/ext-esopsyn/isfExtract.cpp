...
std::pair<DdNode*, DdNode*> IsfExtractManager::expandExactRecur(DdNode* f, DdNode* fc)
{
    ...
    std::tie(f0_new, zf0) = expandExactRecur(f0, fc0);
    std::tie(f1_new, zf1) = expandExactRecur(f1, fc1);
    
	f2 = Cudd_bddXor(_ddManager, f0_new, f1_new); Cudd_Ref(f2);
    
    std::tie(f20_new, zf20) = expandExactRecur(f2, fc0);
    std::tie(f21_new, zf21) = expandExactRecur(f2, fc1);

    uint32_t c0, c1, c20, c21, c_pD, c_nD, c_Sh, c_min;
    c0 = Cudd_CountPathsToNonZero(zf0);
    c1 = Cudd_CountPathsToNonZero(zf1);
    c20 = Cudd_CountPathsToNonZero(zf20);
    c21 = Cudd_CountPathsToNonZero(zf21);
    c_pD = c0 + c21;
    c_nD = c1 + c20;
    c_Sh = c0 + c1;
